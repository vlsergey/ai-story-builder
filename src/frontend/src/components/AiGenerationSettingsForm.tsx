...
export default function AiGenerationSettingsFormWrapper(props: Omit<AiGenerationSettingsFormProps, 'aiEngineDef' | 'defaultAiGenerationSettings'>  ) {
  ...
  const defaultAiGenerationSettings = trpc.settings.allAiEnginesConfig.currentEngine.defaultAiGenerationSettings.get.useQuery()?.data || {}

  if (!currentAiEngineDef) {
    return <span>{t('aiGenerationSettings.noEngine')}</span>
  }

  return <AiGenerationSettingsForm
    {...props}
...
function AiGenerationSettingsForm({ aiEngineDef, className, defaultAiGenerationSettings, value, onChange }: AiGenerationSettingsFormProps): ReactNode {
  const { t } = useLocale()
  ...
  const changeOverriden = useCallback((e: boolean) => {
    setOverriden(e)
    if (e) {
