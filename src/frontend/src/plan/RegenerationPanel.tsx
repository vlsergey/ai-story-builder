...
export default function RegenerationPanel({panelApi}: {panelApi: DockviewPanelApi}) {
  ...
  const handleStart = useCallback((options: RegenerateOptions) => {
  ...
  const statusBadge = () => {
    if (!event?.inProcess) {
      return <span className="inline-flex items-center px-2 py-1 rounded-full text-xs bg-muted text-muted-foreground">{t('regeneration.idle')}</span>
    }
    if (event.stopping) {
      return <span className="inline-flex items-center px-2 py-1 rounded-full text-xs bg-destructive/20 text-destructive-foreground animate-pulse">{t('regeneration.stopping')}</span>
    }
    return <span className="inline-flex items-center px-2 py-1 rounded-full text-xs bg-green-500/20 text-green-600"><div className="w-2 h-2 rounded-full bg-green-500 mr-2 animate-pulse"></div>{t('regeneration.in_progress')}</span>
  }
    ...
    <div className="flex flex-col gap-4 p-4 h-full overflow-y-auto">
      ...
      <ButtonGroup className="shrink-0 w-full">
        <Button
          ...
          type="submit"
          form={formId}
          disabled={event?.inProcess || startMutation.isPending}>
            <PlayIcon />
