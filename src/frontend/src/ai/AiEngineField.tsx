...
export default function AiEngineField({
  className,
  disabled,
  formControl,
  formFieldNamePrefix = "",
  ...
  const fieldRender: ComponentProps<typeof Controller>["render"] = ({
    ...
    switch (field.type) {
      ...
      case "select":
        return (
          <Select aria-invalid={invalid} name={name} value={value ?? ""} onValueChange={onChange} disabled={disabled}>
          ...
          render={({ field, fieldState, fieldState: { error, invalid }, formState }) => (
            <Field orientation={orientation} className={className} data-invalid={invalid}>
              <FieldContent key="content">
                <FieldLabel htmlFor={htmlId} className="whitespace-nowrap">
                  {fieldLabel}
                </FieldLabel>
              </FieldContent>
              {invalid && (
