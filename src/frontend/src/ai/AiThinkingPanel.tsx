...
const AiThinkingPanel = forwardRef<AiThinkingPanelHandle, AiThinkingPanelProps>(({className, itemClassName}, ref) => {
  ...
  return (
    <div className={className ?? "text-muted-foreground"}>
      { items.filter(item => item.type != "message").slice(-3).map((item, index) => {
        ...
        return (<div key={index} className={itemClassName}>
          <div className={className}>
            <div>
              {icon && icon}
            </div>
            <div className="flex-1 ml-1 text-xs">
              <span>{t(`aiThinking.${item.type}${ (item as any)?.action?.type ? '.' + (item as any).action.type : '' }`)}</span>

              { item.type === "custom_tool_call" && (<span>{': '}{item.name}</span>)}
              { item.type === "custom_tool_call" && item.input && (<span>{': '}<code>{item.input}</code></span>)}

              { item.type === "web_search_call" && item.action.type == 'open_page' && (<span>{': '}{item.action.url}</span>)}
              { item.type === "web_search_call" && item.action.type == 'find_in_page' && (<span>{': ' + item.action.pattern + ' @ ' + item.action.url}</span>)}
              { item.type === "web_search_call" && item.action.type == 'search' && (<span>{': '}{item.action.query}</span>)}
            </div>
          </div>
        </div>
      )}) }
