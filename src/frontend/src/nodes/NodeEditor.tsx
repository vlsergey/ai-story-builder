...
export default function NodeEditor<N extends Node>({
  ...
  const onReviewBaseContentChange = useCallback((value: string) => {
  ...
  const onNodeAiGenerationSettingsChange = useCallback((newSettings: Record<string, AiGenerationSettings>) => {
  ...
  const onAiUserPromptChange = useCallback((e: React.ChangeEvent<HTMLTextAreaElement>) => {
    onChange({ ...node, ai_user_prompt: e.currentTarget.value})
  }, [onChange, node])

  const onAiSystemPromptChange = useCallback((e: React.ChangeEvent<HTMLTextAreaElement>) => {
    onChange({ ...node, ai_system_prompt: e.currentTarget.value})
  }, [onChange, node])

  const onAiImproveInstructionsChange = useCallback((e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    <div className="flex flex-col h-full overflow-auto p-4">
      ...
      <Tabs defaultValue='user' className={
        editorMode == 'generate' ?
          ...
          "flex-1 w-full flex flex-col hidden"
      }>
      ...
      >
        <Textarea
          ...
          onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey && !(status == 'GENERATING' || status == 'IMPROVING')) { e.preventDefault(); void handleImprove() } }}
      ...
      >
        <div className="flex justify-end px-2 py-1.5">
          <Button onClick={() => onEditorModeChange('improve')}>
            {tp('improve_with_ai')}
          </Button>
        </div>
      </div>
        ...
        <div className="shrink-0 flex items-center justify-between flex-wrap px-2 py-1 border-t border-border">
          ...
          <div className="flex items-center gap-2">
            {status=='SAVED' && (
              <Button
                variant="secondary"
                onClick={() => { onEditorModeChange('generate'); }}
              >
                {tp('cancel_improve')}
              </Button>
            )}
