...
export const ipcClient = {
  ...
  plan: {
    nodes: () => invoke<any[]>('plan:nodes'),
    getNode: (id: number) => invoke<any>('plan:node:get', id),
    createNode: (data: Record<string, unknown>) => invoke<{ id: number }>('plan:node:create', data),
    patchNode: (id: number, data: Record<string, unknown>) => invoke<{ ok: boolean; word_count?: number | null; char_count?: number | null; byte_count?: number | null }>('plan:node:patch', id, data),
    deleteNode: (id: number) => invoke<{ ok: boolean }>('plan:node:delete', id),
    moveNode: (id: number, data: { parent_id?: number | null }) => invoke<{ ok: boolean }>('plan:node:move', id, data),
    reorderChildren: (child_ids: number[]) => invoke<{ ok: boolean }>('plan:nodes:reorder', { child_ids }),
  },
  graph: {
