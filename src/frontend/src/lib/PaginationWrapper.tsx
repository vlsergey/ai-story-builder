...
const PaginationWrapper = ({
  ...
  return <Pagination {...etc}>
    ...
    { showPrevNext && (zeroBasedValue <= 0 || allDisabled)
      ? <PaginationItem key="-1"><PaginationPrevious/></PaginationItem>
      : <PaginationItem key="-1"><PaginationPrevious onClick={handlePrev}/></PaginationItem>
    }
    { linksToDisplay.map((p: number, index: number) =>
      p === zeroBasedValue
        ? <PaginationItem key={p}><PaginationLink isActive>{p + 1}</PaginationLink></PaginationItem>
        : p === ELLIPSIS_MARK
          ? <PaginationItem key={`_${index}`}><PaginationEllipsis /></PaginationItem>
          : allDisabled
            ? <PaginationItem key={p}><PaginationLink>{p + 1}</PaginationLink></PaginationItem>
    ...
    { showPrevNext && (totalPages !== undefined && zeroBasedValue >= totalPages - 1 || allDisabled)
      ? <PaginationItem key="+1"><PaginationNext /></PaginationItem>
      : <PaginationItem key="+1"><PaginationNext onClick={handleNext}/></PaginationItem>
    }
    </PaginationContent>
  </Pagination>;
};
