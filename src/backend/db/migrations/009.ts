...
export default function migration(db: Database): void {
  ...
  `)
  const rows = db.prepare("SELECT id, content FROM plan_nodes WHERE content IS NOT NULL").all() as {
    id: number
    content: string
  }[]
  const update = db.prepare("UPDATE plan_nodes SET word_count = ?, char_count = ?, byte_count = ? WHERE id = ?")
