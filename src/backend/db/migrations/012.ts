...
export default function migration(db: Database): void {
  ...
  const tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='plan_edges'").all() as {
    name: string
  }[]
  if (tables.length === 0) {
    db.exec(`
    ...
    const rows = db
      .prepare(`SELECT id, parent_id, position FROM plan_nodes WHERE parent_id IS NOT NULL`)
