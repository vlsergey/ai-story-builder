...
import { RegenerationContainerContext, RegenerationNodeContext, PlanNodeAiGenerationStatus } from './generate/RegenerationContext.js'
import { RegenerateOptions } from '../../../shared/RegenerateOptions.js'

export function aiRegenerateNodeContentWatchAndReview(nodeId: number, options: RegenerateOptions): Observable<DataOrEventEvent<PlanNodeRow, ResponseStreamEvent>, unknown> {
  ...
  return toObservable<DataOrEventEvent<PlanNodeRow, ResponseStreamEvent>>(async (emit) => {
    const asContainer: RegenerationNodeContext['asContainer'] = async <T>(multiplier: number, block: (context: RegenerationContainerContext) => Promise<T>) => {
      return await block({
        options,
        onNodeStart: async (node, block: (context: RegenerationNodeContext) => Promise<{ result: T, status: PlanNodeAiGenerationStatus }>) => {
    ...
    const mainNodeContext : RegenerationNodeContext = {
      ...
      onData: (data) => { emit.next({ type: 'data', data }); },
      onEvent: (event) => { emit.next({ type: 'event', event }); },
      asContainer,
    }
