...
- `plan_nodes`
  ...
  - `type` TEXT NOT NULL DEFAULT `'text'` — `'text'` | `'lore'` | `'merge'` | `'split'`
  ...
  - `node_type_settings` TEXT — JSON object with node-type-specific settings. Structure depends on `type`:
    - For `type='merge'` (merge node):
    ...
    - For `type='split'` (splitter node):
      - `strategy` TEXT — splitting strategy: `'separator'` or `'AI'`
      - `separator` TEXT (optional) — separator text used when strategy is `'separator'`
