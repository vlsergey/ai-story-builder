...
* **Sync with AI Engine** — always active; synchronises all lore with the selected AI Engine and permanently removes items marked for deletion. Actual behavior depends on engine:
  * **Yandex Cloud AI**: upload new/changed non-empty nodes as plain-text files (one file per lore node) to the Yandex Files API; delete remote files for nodes that became empty or are marked `to_be_deleted`; **delete the old SearchIndex before creating the new one** (to avoid orphaned indexes); create a new SearchIndex from all current file IDs and poll until it reaches `DONE` state (creation is async); only after the new index is fully created — mark all nodes as synced and store the new `search_index_id` in project settings. If the new index fails to create, clear `search_index_id` so the next sync starts fresh.
